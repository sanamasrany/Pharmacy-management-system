...
class Add_dispenser_warehouse extends StatelessWidget {


  var dispenserEmailController = TextEditingController();
  ...
  return  BlocConsumer<WarehouseAddEmpAndDisCubit,WarehouseAddEmpAndDisStates>(
      ...
      if(state is WarehouseAddDisSuccessState)//نجاح او فشل التسجيل
        ...
        if(state.value==200) {
          ...
          showToast(text:"Dispenser has been created." ,state: ToastStates.SUCCESS);
      ...
      if(state is WarehouseAddDisErrorState){
        ...
        if(state.error==422){
          showToast(text:"The selected dispenser email is invalid." ,state: ToastStates.EROOR);
    ...
    controller: dispenserEmailController,
    type: TextInputType.emailAddress,
    validate: (String? value) {
    if (value!.isEmpty) {
    return "dispenser email must not be empty";
    }
    return null;
    },
    onSubmit: (value) {},
      ...
      ConditionalBuilder(
        ...
        builder: (context) =>    newDefaultButton(
            ...
            function: (){
              ...
              WarehouseAddEmpAndDisCubit.get(context).adddispensertowarehouse( dispensereemail: dispenserEmailController.text);
